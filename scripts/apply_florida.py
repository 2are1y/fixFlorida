...
def insert_before_once(path: Path, marker: str, insertion: str, sentinel: str) -> None:
    ...
    if sentinel_count:
        if sentinel_count != 1:
            raise PatchError(
                f"Expected one existing sentinel in {path}, found {sentinel_count}:\n{sentinel}"
            )
    ...
    if marker_count != 1:
        raise PatchError(
            f"Expected exactly one insertion marker in {path}, found {marker_count}:\n{marker}"
        )
...
def verify_source_transformations(core: Path, gum: Path) -> None:
    ...
    assert_contains(embed, "subprocess.run([", expected=2)
