...
def validate_entrypoint_before(policy: Policy, binary: object) -> dict[str, int]:
    ...
    if policy.entrypoint == "require-frida-agent-main":
        ...
        if dynamic_names and state["dynamic_old_before"] == 0:
            raise RuntimeError(
                "frida_agent_main exists only outside .dynsym; the injectable exported "
                "entrypoint was not found"
            )
    elif state["old_before"] == 0 and state["main_before"] == 0:
        raise RuntimeError(
...
def validate_entrypoint_after(
    ...
    elif policy.entrypoint == "allow-prepatched-main" and main_after == 0:
...
def sanitize_inactive_entrypoint_bytes(data: bytes, used_replacements: set[bytes]) -> tuple[bytes, int]:
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        data, inactive_entrypoint_hits = sanitize_inactive_entrypoint_bytes(data, used_replacements)
